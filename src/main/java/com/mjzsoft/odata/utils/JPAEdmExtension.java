	...
	public void extendJPAEdmSchema(JPAEdmSchemaView view) {
		...
		for (EntityType entityType : edmSchema.getEntityTypes()) {
			for (Property property : entityType.getProperties()) {
				...
				if (i18n != null) {
					try {
						label = i18n.getString(entityType.getName() + "." + property.getName());
					} catch (Exception e) {
					}
				}
				List<AnnotationAttribute> annotationAttributeList = new ArrayList<AnnotationAttribute>();
				if (label != null) {
					annotationAttributeList.add(new AnnotationAttribute().setNamespace(SAP_NAMESPACE)
							.setPrefix(SAP_PREFIX).setName(LABEL).setText(label));
				}
			...
			if (field.getName().equals(((JPAEdmMappingImpl) property.getMapping()).getInternalName())) {
				if (field.getAnnotation(Sap.class) != null) {
					...
					for (Entry<String, Object> memberValue : memberValues.entrySet()) {
						result.add(new AnnotationAttribute().setNamespace(SAP_NAMESPACE).setPrefix(SAP_PREFIX)
								.setName(memberValue.getKey()).setText(String.valueOf(memberValue.getValue())));
					}
				}
			}
		}
	...
	private void addSmartAnnotations(final Schema edmSchema) {
		...
		for (final EntityContainer container : edmSchema.getEntityContainers()) {
			for (final EntitySet entitySet : container.getEntitySets()) {
				schemaAnnotations.add(new AnnotationElement().setName("Annotations")
						.setAttributes(new ArrayList<AnnotationAttribute>() {
							...
							{
								add(new AnnotationElement().setName("Annotation")
										.setAttributes(new ArrayList<AnnotationAttribute>() {
											{
											...
											{
												add(new AnnotationElement().setName("Collection")
														.setChildElements(new ArrayList<AnnotationElement>() {
															{
																for (final Field field : getFieldsOfEntitySet()) {
																	if (field
																			.getAnnotation(SAPLineItem.class) != null) {
																		add(new AnnotationElement().setName("Record")
																				.setAttributes(
																										...
																										.setAttributes(
																												new ArrayList<AnnotationAttribute>() {
																													...
																													private String getEdmNameOfField() {
																														for (EntityType entityType : edmSchema
																																.getEntityTypes()) {
																															if (field
																																	.getDeclaringClass()
																																	.equals(((JPAEdmMappingImpl) entityType
																																			.getMapping())
																																					.getJPAType())) {
																																for (Property property : entityType
																																		.getProperties()) {
																																	if (field
																																			.getName()
																																			.equals(((JPAEdmMappingImpl) property
																																					.getMapping())
																																							.getInternalName())) {
																																		return property
																																				.getName();
															...
															private Field[] getFieldsOfEntitySet() {
																for (EntityType entityType : edmSchema
																	...
																	if (entityType.getName().equals(
																		...
																		return ((JPAEdmMappingImpl) entityType
																				.getMapping()).getJPAType()
																						.getDeclaredFields();
																	}
																}
																return null;
